...
class RedisServer{
	...
	{
		//$re=$this->_send(array('info'));
		//list(,$val)=explode($value.':',$re);
		//$val=explode("\r\n",$val);
	...
	{
		return $this->_send(array('hIncrBy', $key, $field, (int)$increment));
	}
	
	public function hIncrByFloat($key, $field, $increment)
	{
		return $this->_send(array('hIncrByFloat', $key, $field, (float)$increment));
	}
	...
	public function zAdd($key, $score, $member = NULL)
	{
		...
		{
			foreach ($score as $member => $score_value )
			{
				$values[] = $score_value;
				$values[] = $member;
			}
